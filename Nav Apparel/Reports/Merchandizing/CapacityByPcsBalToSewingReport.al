report 51338 CapacityByPcsBalToSewingReport
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    Caption = 'Month Wise Sewing Balance Report';
    RDLCLayout = 'Report_Layouts/Merchandizing/CapacityByPcsBalToSewingReport.rdl';
    DefaultLayout = RDLC;

    dataset
    {
        dataitem(BuyerWiseOrderBookinBalatoSew; BuyerWiseOrderBookinBalatoSew)
        {
    ...
    begin
        if YearVar >= 2023 then
            CodeUnit2Rec.CapacityByPcsReports(YearVar, 2)
        else
            Error('Invalid date.');
    end;
