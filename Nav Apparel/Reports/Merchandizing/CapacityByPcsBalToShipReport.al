report 51339 CapacityByPcsBalToShipReport
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    Caption = 'Month Wise Ship Balance Report';
    RDLCLayout = 'Report_Layouts/Merchandizing/CapacityByPcsBalToShipReport.rdl';
    DefaultLayout = RDLC;

    dataset
    {
        dataitem(BuyerWiseOrderBookinBalatoShip; BuyerWiseOrderBookinBalatoShip)
        {
    ...
    begin
        if YearVar >= 2023 then
            CodeUnit2Rec.CapacityByPcsReports(YearVar, 3)
        else
            Error('Invalid date.');
    end;
