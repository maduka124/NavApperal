report 51400 "Check-Customized"
{
    DefaultLayout = RDLC;
    //RDLCLayout = './CheckCustomized.rdlc';
    Caption = 'Check-New';
    ...
    var
        ...
        DecimalFirst: Text;
        DecimalLast: Text;
        OnesTextDecimalFirst: array[20] of Text[30];
        TensTextDecimalFirst: array[10] of Text[30];
        FinalCents: array[20] of Text[30];
    ...
    local procedure AddToNoText(var NoText: array[3] of Text[50]; var NoTextIndex: Integer; var PrintExponent: Boolean; AddText: Text[30])
    begin
        PrintExponent := TRUE;
    ...
    procedure FormatNoTextCustomized(var NoText: array[2] of Text[50]; No: Decimal; CurrencyCode: Code[10])
    var
        PrintExponent: Boolean;
        Ones: Integer;
        Tens: Integer;
        ...
        NoTextIndex: Integer;
        DecimalPosition: Decimal;
        ConvertIntFirst: Integer;
        ConvertIntLast: Integer;
    begin
        CLEAR(NoText);
        NoTextIndex := 1;
        NoText[1] := ''; //KI
        ...
        AddToNoText(NoText, NoTextIndex, PrintExponent, Text028);
        DecimalPosition := GetAmtDecimalPosition;
        //AddToNoText(NoText,NoTextIndex,PrintExponent,(FORMAT(No * DecimalPosition) + '/' + FORMAT(DecimalPosition))); //OLD

        //TD(-)
        DecimalFirst := COPYSTR(FORMAT(No * DecimalPosition), 1, 1);
        DecimalLast := COPYSTR(FORMAT(No * DecimalPosition), 2, 1);
        IF (DecimalFirst <> '') AND (DecimalLast = '') THEN
            EVALUATE(ConvertIntFirst, DecimalFirst)
        ELSE
            IF (DecimalFirst <> '') AND (DecimalLast <> '') THEN BEGIN
                EVALUATE(ConvertIntLast, DecimalLast);
                EVALUATE(ConvertIntFirst, DecimalFirst);
            END;

        IF (ConvertIntFirst = 0) AND (ConvertIntLast = 0) THEN
            OnesTextDecimalFirst[1] := ''   //KI ZERO
        ELSE
            IF (ConvertIntFirst > 0) AND (DecimalLast = '') THEN
                OnesTextDecimalFirst[1] := OnesText[ConvertIntFirst]
            ELSE
                IF (ConvertIntFirst > 0) AND (ConvertIntLast = 0) THEN BEGIN
                    IF (ConvertIntFirst = 1) THEN
                        OnesTextDecimalFirst[1] := OnesText[10]
                    ELSE
                        IF (ConvertIntFirst > 1) THEN
                            OnesTextDecimalFirst[1] := TensText[ConvertIntFirst];
                END
                ELSE
                    IF (ConvertIntFirst > 0) AND (ConvertIntLast > 0) THEN BEGIN
                        IF (ConvertIntFirst = 1) THEN
                            OnesTextDecimalFirst[1] := OnesText[10 + ConvertIntLast]
                        ELSE
                            IF (ConvertIntFirst > 1) THEN
                                OnesTextDecimalFirst[1] := TensText[ConvertIntFirst] + '-' + OnesText[ConvertIntLast];
                    END;

        IF (No = 0) OR (No = 1) THEN
            FinalCents[1] := OnesTextDecimalFirst[1] + ' ONLY' //PS CENTS
        ELSE
            FinalCents[1] := OnesTextDecimalFirst[1] + ' ONLY'; //PS CENTS

        //AddToNoText(NoText,NoTextIndex,PrintExponent,('.'+FORMAT(No * DecimalPosition)));

        AddToNoText(NoText, NoTextIndex, PrintExponent, FinalCents[1]);
        //TD(+)
