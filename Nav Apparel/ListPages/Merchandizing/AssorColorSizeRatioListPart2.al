page 71012675 AssorColorSizeRatioListPart2
{
    PageType = ListPart;
    //AutoSplitKey = true;
    ...
    layout
    {
        area(Content)
        {
            repeater(General)
            {
                ...
                field("SHID/LOT Name"; "SHID/LOT Name")
                {
                    ApplicationArea = All;
                    Visible = false;
                }
        ...
        area(Processing)
        {
            action("Quantity Breakdown")
            {
                ...
                begin
                    ...
                    if not AssorColorSizeRatioRec.FINDSET() then
                        ...
                        repeat
                            ...
                            if AssorColorSizeRatioViewRec.FINDSET() then begin
                                ...
                                AssorColorSizeRatioViewRec."Style No." := "Style No.";
                                AssorColorSizeRatioViewRec."PO No." := PONO;
                                AssorColorSizeRatioViewRec."lot No." := "lot No.";
                    ...
                    BOMEstimateCostRec.SetRange("Style No.", "Style No.");
                    if not BOMEstimateCostRec.FindSet() then
                        Error('Cannot find Estimate costing for the style: %1', "Style No.");

                    if AssorColorSizeRatioViewRec.FINDSET() then begin
                        repeat
                            AssorColorSizeRatioPriceRec.Init();
                            AssorColorSizeRatioPriceRec."Style No." := "Style No.";
                            AssorColorSizeRatioPriceRec."PO No." := PONO;
                            AssorColorSizeRatioPriceRec."lot No." := "lot No.";
